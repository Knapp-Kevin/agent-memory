...
**Status**: contract version `1.0.0`, Sprint 4a (plan `docs/plan-sprint4a-public-api-contract.md`, ledger Entry #45). Implements PRD-001 R1's proposal, decision, approval, commit, retrieval-candidate and recall-admission stages at the boundary; action authority and execution evidence follow in Sprint 4c; the JS runtime's conformance follows in Sprint 4b.

## What the contract is

An embedding host calls five functions in `agentmem_ref.api.surface`, each taking a `GovernedMemoryAdapter` and an envelope, and each returning a **result envelope**. The envelopes are schema-backed and carry a **contract version**; the adapter's internal dataclasses (`Proposal`, `RecallContext`, `CommitResult`, `AdmissionResult`) stay behind the surface and are not the contract.
...
Examples a consumer can start from: `reference/fixtures/api/proposal-envelope.example.json`, `reference/fixtures/api/recall-context.example.json`.
...
|---|---|---|
| `current` | same major, minor not lower | the stage |
| `migration_required` | same major, lower minor | nothing; `stage: none` |
...
Action authority (`procedural_memory.apply_action_governance`) and execution evidence (`record_runtime_execution`), history and provenance inspection, and posture inspection have no envelope form yet. PRD-001 R1 lists them; they need their own schemas and are not in contract `1.0.0`.
